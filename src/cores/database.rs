...
impl Database {
    ...
    pub fn create_snapshot(&self) -> Result<SnapshotStats, Box<dyn Error>> {
        ...
        let index = self.index.read().map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::Other, "Index lock is poisoned")
        })?;
        self.persist_as_binary(self.dir.join(INDEX_FILE), &*index)?;

        let storage = self.storage.read().map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::Other, "Storage lock is poisoned")
        })?;
...
impl DatabaseService for Arc<Database> {
    ...
    async fn insert(
        ...
        let mut storage = self
            .storage
            .write()
            .map_err(|_| Status::internal("Storage lock is poisoned"))?;
        storage.insert(&id, &record)?;

        let mut index = self
            .index
            .write()
            .map_err(|_| Status::internal("Index lock is poisoned"))?;

        if let Err(status) = index.insert(&id, &record, storage.records()) {
            storage.delete(&id)?;
            return Err(status);
        }
