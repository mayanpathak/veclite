...
///
/// veclite uses a modified version of IVF index algorithm. This custom index
/// implementation allows veclite to maintain a balanced index structure
...
impl Index {
    ...
    pub fn insert(
        ...
        if self.clusters[nearest_centroid].len() < self.density {
            ...
            self.clusters[nearest_centroid].push(*id);
            if let Err(status) = self.split_cluster(&nearest_centroid, records) {
                self.clusters[nearest_centroid]
        ...
        for cluster_id in nearest_clusters.iter().take(probes) {
            for record_id in &self.clusters[*cluster_id] {
                ...
                let distance = match distance {
                    Some(distance) if distance.is_finite() => distance as f32,
                    _ => continue,
                };
    ...
    fn split_cluster(
        ...
        let vectors = record_ids
            .iter()
            .map(|id| {
                records.get(id).map(|record| &record.vector).ok_or_else(|| {
                    Status::internal(format!(
                        "Index references missing record: {id}"
                    ))
                })
            })
            .collect::<Result<Vec<&Vector>, Status>>()?;

        let mut kmeans = KMeans::new(2).with_metric(self.metric);
        kmeans.fit(Rc::from(vectors)).map_err(|e| {
            Status::internal(format!("Failed to split index cluster: {e}"))
        })?;
